	...
	<channel>
		...
		while( count($EM_Events) > 0 ){
			foreach ( $EM_Events as $EM_Event ) {
				...
				$event_location = $EM_Event->output('#_ATT{location-name}');
				$event_time = $EM_Event->output('#_ATT{date-time}');
				?>
				<item>
                    ...
                    <location><?php echo $event_location; ?></location>
                    <time><?php echo $event_time; ?></time>
				</item>
				<?php
				$count++;
			}
